...
def main():
    ...
    moon = Body(-1 * AU - 384e6, 0,  # earth.x + distance from earth
                radius=1.737e3, color=WHITE, mass=7.347e22, initial_velocity=(0, 29.78e3 + 1.022e3))
    ...
    uranus = Body(-19.191 * AU, 0, radius=25.362e3, color=WHITE, mass=8.681e25, initial_velocity=(0, 6.81e3))
    neptune = Body(-30.069 * AU, 0, radius=24.622e3, color=LIGHT_BLUE, mass=1.024e26, initial_velocity=(0, 5.43e3))

    celestial_bodies = [sun, earth, mars, mercury, venus, jupiter, saturn, uranus, neptune, moon]
    ...
    while run:
        ...
        for body in celestial_bodies:
            ...
            body.draw(WINDOW)
        d = FONT.render(f"d_ME: {round(math.sqrt((earth.x - moon.x)**2 + (earth.y - moon.y)**2)/1000000, 1)}e3km",
                        True, WHITE)
        WINDOW.blit(d, (10, 10))
